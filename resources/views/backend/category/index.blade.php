...
<main class="page-content">
    ...
    <div class="col-auto pb-4">
      <div class="d-flex align-items-center gap-2 justify-content-lg-end">
        <a href="{{route('category.create')}}"> <button class="btn btn-primary px-4"><i class="bi bi-plus-lg me-2"></i>Add
            Category</button></a>
      </div>
    </div>
    </div><!--end row-->
      ...
      <div class="card-body">
        <div class="customer-table">
          <div class="table-responsive white-space-nowrap">
            <table id="categoryList" class="table table-bordered align-middle" style="width:100%">
...
var reporttables =  $('#categoryList').DataTable({
  "order": [[ 0, "desc" ]],
      "serverSide": true,
  "stateSave": true,
      'processing': true,
  ...
  },
  {
    ...
    "render": function ( data, type, row, meta ) {
      return '<p class="mb-0 customer-name fw-bold">'+row.name+'</p>';
    }
  },
  {
    "targets": 2,
    orderable: false,
    "render": function ( data, type, row, meta ) {
         return'<div class="toggle-btn '+(row.status=="active"?"active":"")+' toggle_button_'+row.id+'" > <input type="checkbox" class="cb-value" data-id='+row.id+'  id="category_status_id" name="category_status_id" '+(row.status=="active" ? "checked=true" :"false")+' data-category_status_id="'+row.id+'"><span class="round-btn '+(row.status=="active"?"active":"")+'  toggle_button_'+row.id+'""></span> </div>';
  ...
  },
  {
    ...
    "render": function ( data, type, row, meta ) {
      ...
      return '<a href="category/'+row.id+'/edit"><button class="btn btn-sm btn-light border" type="button"><i class="bi bi-pencil-square text-warning"></i></button></a><a><button class="btn btn-sm btn-light border" onclick="deleteOrder('+row.id+')"type="button"><i class="bi bi-trash3-fill text-danger"></i></button></a>';
    ...
    $.ajax({
      ...
      success: function (res) {
        if (res) {
          ...
          swal("Category Delete Failed", "Please try again. :)", "error");
        }
      }
    });
...
<script>
  $(document).on("click","#category_status_id",function(){
    var category_status_id = $(this).data('category_status_id');
    var formData = new FormData();
    formData.append('id',category_status_id);
    $.ajax({
      url:"{{route('category_status_update')}}",
      method:'POST',
      data:formData,
      contentType:false,
      processData:false,
