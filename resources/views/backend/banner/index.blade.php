...
var reporttables =  $('#bannerList').DataTable({
  ...
  },
  {
    ...
    "render": function ( data, type, row, meta ) {
      return '<img src="../public/'+row.image[0]+'" width=100 height=100>';
    }
  },
  {
    "targets": 2,
    ...
    //  return' <div id="app-cover"><div class="toggle-button-cover"><div class="button-cover"><div class="button r" id="button-1"><input type="checkbox" data-banner_id='+row.id+' checked=true id="banner_status_id" name="banner_status_id" class="checkbox" value='+row.id+' /><div class="knobs"></div><div class="layer"></div> </div></div></div></div>';
     ...
     return'<div class="toggle-btn '+(row.status=="active"?"active":"")+' toggle_button_'+row.id+'" > <input type="checkbox" class="cb-value" data-id='+row.id+'  id="banner_status_id" name="banner_status_id" '+(row.status=="active"?"checked=true":"false")+' data-banner_id='+row.id+'><span class="round-btn '+(row.status=="active"?"active":"")+'  toggle_button_'+row.id+'""></span> </div>';
...
function(isConfirm){
  ...
  $(document).on("click","#banner_status_id",function(){
    ...
    $.ajax({
      url:"{{route('banner_status_update')}}",
      method:'POST',
      data:form_data,
      contentType:false,
      processData:false,
